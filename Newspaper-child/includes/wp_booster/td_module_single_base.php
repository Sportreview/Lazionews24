...
//        ){
        ...
        if (get_post_format($this->post->ID) == 'video') {
            ...
            if (!is_null($this->post_thumb_id)) {
                ...
                if (!empty($featured_image_info['caption'])) {
                    $buffy .= '<figure>';
                    $buffy .= $image_html;
                    $buffy .= '<figcaption class="wp-caption-text">&copy; foto ' . $featured_image_info['caption'] . '</figcaption>';
                    $buffy .= '</figure>';
                } else {
                    $buffy .= '<figure>';
                    $buffy .= $image_html;
                    $buffy .= '<figcaption class="wp-caption-text">&copy; foto www.imagephotoagency.it</figcaption>';
                    $buffy .= '</figure>';
                }
                $buffy .= '</div>';
    ...
    function get_next_prev_posts() {
        ...
        $imgprecedente = get_the_post_thumbnail(( $prev_post->ID ), 'td_100x70');
        $imgsuccessiva = get_the_post_thumbnail(( $next_post->ID ), 'td_100x70');
        ...
        if (!empty($next_post) or !empty($prev_post)) {
            ...
            if (!empty($prev_post)) {
                ...
                $buffy .= '<div class="td-post-next-prev-content">';
                $buffy .= '<div class="post-prev-next">';
                $buffy .= '<i class="fa fa-arrow-circle-left" aria-hidden="true"></i><span>' .__td('Previous article', TD_THEME_NAME) . '</span>';
                $buffy .= '</div>';
                $buffy .= '<a class="post-prev-thumb" href="' . esc_url(get_permalink($prev_post->ID)) . '">'.$imgprecedente.'</a>';
            ...
            if (!empty($next_post)) {
                ...
                $buffy .= '<div class="td-post-next-prev-content">';
                $buffy .= '<div class="post-prev-next">';
                $buffy .= '<span>' .__td('Next article', TD_THEME_NAME) . '</span><i class="fa fa-arrow-circle-right" aria-hidden="true"></i>';
                $buffy .= '</div>';
                $buffy .= '<a class="post-next-thumb" href="' . esc_url(get_permalink($next_post->ID)) . '">'.$imgsuccessiva.'</a>';
